...
// CONSTRUCTOR
// -------------------------
Batalla::Batalla(Entrenador &ent1, Entrenador &ent2, IGenerador* gen): entrenador_1(ent1),
    entrenador_2(ent2),
...
std::string Batalla::atacar(Entrenador &atacante, Movimiento &ataque)
{
  std::string mensaje = "";
  Entrenador defensor = entrenador_1;

  if (!esTurnoDe(atacante)) {
    return format("No es tu turno, {}!", atacante.getNombre());
  } // verifica que sea el turno del entrenador

  if (atacante.getPokemonActivo().isDebil())
  {
    return format(":prohibited: {} está debilitado :prohibited:", atacante.getPokemonActivo().getNombre());
  ...
  {
    return format(":prohibited: **{}** está debilitado :prohibited:", defensor.getPokemonActivo().getNombre());
  } // Tampoco ataca si el pokemon del defensor está debil

  if (ataque.esEspecial() && atacante.getContadorEspecial() != 0)
  {
    return format(":prohibited: Debes esperar **{}** turnos más para usar un ataque especial :prohibited:", atacante.getContadorEspecial());
  } // verifica que pueda usar ataque especial

  if (ataque.getCurrentPP() == 0)
  {
    return format("**{}** está agotado:exclamation:", ataque.getNombre());
...
std::string Batalla::seleccionarPokemon(Entrenador &ente, Pokemon &pok) {
    Pokemon copiaPokemon = pok;
...
bool Batalla::finBatalla() {
    for (Pokemon p : entrenador_1.getPokemons()) {
        if (!p.isDebil()) {return false;}
    }
    for (Pokemon p : entrenador_2.getPokemons()) {
        if (!p.isDebil()) {return false;}
    }
    return true;
}
