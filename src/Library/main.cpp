...
bot.on_slashcommand([&bot](const dpp::slashcommand_t& event) {
    ...
    if (event.command.get_command_name() == "atacar") {
        try {
            std::string nombreUsuario = event.command.usr.username;
            std::string nombreMov = std::get<std::string>(event.get_parameter("ataque"));
...
bot.on_ready([&bot](const dpp::ready_t& event) {
    if (dpp::run_once<struct register_bot_commands>()) {
        ...
        dpp::slashcommand pokedex("pokedex", "Desplegar pokemones disponibles", bot.me.id);

        dpp::slashcommand atacar("atacar", "Ejecuta un ataque", bot.me.id);
        atacar.add_option(
            dpp::command_option(dpp::co_string, "ataque", "Nombre del ataque", true) // Parámetro obligatorio
        );

        // Crear un vector de comandos
        std::vector<dpp::slashcommand> comandos = { unirme, llamar, batalla, pokedex, atacar};
